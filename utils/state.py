"""Streamlit session-state management for Resin Chatbot conversations."""
...
def get_user_name() -> str:
    """Return the user's chosen display name."""

    return str(st.session_state.get("user_name", "")).strip()


def set_user_name(name: str) -> None:
    """Store the user's chosen display name for the current session."""
...
def add_message(
    ...
    try:
        ...
        logger.error(
            "Could not persist chat message: %s",
            type(error).__name__,
        )
